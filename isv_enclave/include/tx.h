///Source code from https://github.com/keeshux/basic-blockchain-programming
#ifndef __TX_H
#define __TX_H
...
void bbp_txout_create_p2pkh(bbp_txout_t *txout, const uint64_t value, const char *hash160) {
    char script[52] = { 0 };
    snprintf(script, sizeof(script),"76a914%s88ac", hash160);
...
void bbp_txin_create_p2pkh(bbp_txin_t *txin, const bbp_outpoint_t *outpoint,
    ...
    char script[400] = { 0 };
    snprintf(script,sizeof(script), "%02lx%s%02x%02lx%s", strlen(sig) / 2 + 1, sig, flag, strlen(pub) / 2, pub);
...
void bbp_txin_create_signable(bbp_txin_t *txin, const bbp_outpoint_t *outpoint, const bbp_txout_t *utxo) {
    ...
    txin->script = static_cast<uint8_t *>(malloc(utxo->script_len));
