...
sgx_status_t create_address_from_string(sgx_enclave_id_t eid, uint8_t* addr, size_t len)
{
	sgx_status_t status;
	ms_generateBitcoinAddr_ini_t ms;
	ms.ms_addr = addr;
	ms.ms_len = len;
...
sgx_status_t generateAddr(sgx_enclave_id_t eid)
{
	sgx_status_t status;
	status = sgx_ecall(eid, 5, &ocall_table_isv_enclave, NULL);
	return status;
}

sgx_status_t generate_transaction(sgx_enclave_id_t eid)
{
	sgx_status_t status;
	status = sgx_ecall(eid, 6, &ocall_table_isv_enclave, NULL);
	return status;
}

sgx_status_t set_initial_remain(sgx_enclave_id_t eid, int initial[1])
{
	sgx_status_t status;
	ms_set_initial_remain_t ms;
	ms.ms_initial = (int*)initial;
	status = sgx_ecall(eid, 7, &ocall_table_isv_enclave, &ms);
	return status;
}

sgx_status_t return_size(sgx_enclave_id_t eid, size_t* retval)
{
	sgx_status_t status;
	ms_return_size_t ms;
...
sgx_status_t gen_trans_record(sgx_enclave_id_t eid, const unsigned char* addr, int out[1])
{
	sgx_status_t status;
	ms_gen_trans_record_t ms;
	ms.ms_addr = addr;
	ms.ms_out = (int*)out;
...
sgx_status_t seal(sgx_enclave_id_t eid, sgx_sealed_data_t** retval)
{
	sgx_status_t status;
	ms_seal_t ms;
